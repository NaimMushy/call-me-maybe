...
def verify_files(fc_def_path: str, pr_path: str) -> [list[dict], list[dict]]:

    with open(fc_def_path) as func_file:

        functions = json.load(func_file)
...
class Constraint(BaseModel):
    ...
    def get_parameters(self, slm: Small_LLM_Model, prompt: dict, function_name: str) -> dict[str, str | float | bool]:
        ...
        Here is your user request: {prompt}. \
